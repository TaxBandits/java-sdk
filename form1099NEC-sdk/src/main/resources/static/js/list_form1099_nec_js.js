$(document).ready(function() {
    ...
    $("#pEmptyForm1099NEC").html('There are no 1099-NEC forms created under the business <b>' + stringOrHyphen(business.businessNm) + '</b>. Click "Create Form1099NEC" to create a new Form 1099-NEC.');

    // Initial method call
    updateView(INITIAL);
    getForm1099NecList(business)

    //  Button click handlers
    $("#btnCreateForm1099Nec").click(function() {
        navigateToCreateForm1099Nec(business, ADD, "", "", "");
    });

    $("#btnEmptyCreateForm1099Nec").click(function() {
        navigateToCreateForm1099Nec(business, ADD, "", "", "");
    });
...
function getForm1099NecList(business) {
    $("#pageProgressBar").show();

    var request = { businessId: business.businessId };
    $.ajax({
        url: form1099NecListEndPoint,
        type: 'POST',
        ...
        success: function (response) {
            ...
            if(is200(code)) {
                ...
                $.each(form1099Records, function(index, form1099Record) {
                    var btnEditForm1099NecId = "btnEditForm1099NecId" + index;
                    ...
                    rows += '<tr>'+
                                ...
                                '<td style="word-break: break-all">'+
                                    '<div class="dropdown dropdown-scroll action d-md-flex justify-content-between align-items-center px-4">'+
                                    	'<button class="btn btn-primary status-btn btn_smm" id="' + btnEditForm1099NecId + '"><i class="fa fa-edit"></i></button>'+
                ...
                $.each(form1099Records, function(index, form1099Record) {
                    ...
                    var btnEditForm1099NecId = "btnEditForm1099NecId" + index;
                    $("#" + btnEditForm1099NecId).click(function () {
                        navigateToCreateForm1099Nec(business, EDIT, submissionId, recordId, recipientId);
                    });

                    var btnViewStatusId = "btnViewStatusId" + index;
                    $("#" + btnViewStatusId).click(function () {
                        form1099NecViewStatus(submissionId);
                    });
                    ...
                    $("#" + btnDeleteId).click(function () {
                        deleteForm1099Nec(submissionId, recordId);
                    });
...
//  Form1099Nec status Transmit
function form1099NecViewStatus(submissionId) {
    $("#pageProgressBar").show();

    var request = { submissionId: submissionId };
    $.ajax({
        url: form1099NecStatusEndPoint,
        type: 'POST',
        data: JSON.stringify(request),
        contentType: 'application/json; charset=utf-8',
        success: function (response) {
            ...
            if(is200(code)) {
                ...
                $("#listForm1099NecStatusTBody").html(htmlStatus);
...
//  Request draft Pdf URLs of the Form1099Nec Transmit
...
//  Request Pdf URLs of the Form1099Nec Transmit
function requestPdfURLs(submissionId, recordId) {
    ...
    $.ajax({
        ...
        success: function (response) {
            $("#pageProgressBar").hide();
            var data = response.data;
            var errorRecords = data.form1099NecRecords.errorRecords;
            var successRecords = data.form1099NecRecords.successRecords;
...
//  Delete Form1099Nec
function deleteForm1099Nec(submissionId, recordId) {
    $("#pageProgressBar").show();
...
function updateView(type) {
    if(isInitial(type)) {
        $("#emptyForm1099NECDiv").hide();
        $("#form1099NecListDiv").hide();
        $("#form1099NecListTable").hide();
        $("#backForm1099NecList").hide();
    } else if(isAvailableData(type)) {
        $("#emptyForm1099NECDiv").hide();
        $("#form1099NecListDiv").show();
        $("#form1099NecListTable").show();
        $("#backForm1099NecList").show();
    } else {
        $("#emptyForm1099NECDiv").show();
        $("#form1099NecListDiv").hide();
        $("#form1099NecListTable").hide();
        $("#backForm1099NecList").hide();
    }
