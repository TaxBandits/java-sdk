$(document).ready(function() {
    ...
    $("#pEmptyForm1099MISC").html('There are no 1099-MISC forms created under the business <b>' + stringOrHyphen(business.businessNm) + '</b>. Click "Create Form1099MISC" to create a new Form 1099-MISC.');

    // Initial method call
    updateView(INITIAL);
    getForm1099MiscList(business)

    //  Button click handlers
    $("#btnCreateForm1099Misc").click(function() {
        navigateToCreateForm1099Misc(business, ADD, "", "", "");
    });

    $("#btnEmptyCreateForm1099Misc").click(function() {
        navigateToCreateForm1099Misc(business, ADD, "", "", "");
    });
...
function getForm1099MiscList(business) {
    $("#pageProgressBar").show();

    var request = { businessId: business.businessId };
    $.ajax({
        url: form1099MiscListEndPoint,
        type: 'POST',
        ...
        success: function (response) {
            ...
            if(is200(code)) {
                ...
                $.each(form1099Records, function(index, form1099Record) {
                    var btnEditForm1099MiscId = "btnEditForm1099MiscId" + index;
                    ...
                    var isEIN = form1099Record.recipient.tintype === "EIN";
                    var firstPayeeNm = !isEIN && isValidString(form1099Record.recipient.firstNm) ? form1099Record.recipient.firstNm + " " + form1099Record.recipient.lastNm : form1099Record.recipient.recipientNm;
                    rows += '<tr>'+
                                ...
                                '<td class="text-start" style="word-break: break-all">' + firstPayeeNm + '</td>'+
                                '<td style="word-break: break-all">'+
                                    '<div class="dropdown dropdown-scroll action d-md-flex justify-content-between align-items-center px-4">'+
                                	    '<button class="btn btn-primary status-btn btn_smm" id="' + btnEditForm1099MiscId + '"><i class="fa fa-edit"></i></button>'+
                ...
                $.each(form1099Records, function(index, form1099Record) {
                    ...
                    var btnEditForm1099MiscId = "btnEditForm1099MiscId" + index;
                    $("#" + btnEditForm1099MiscId).click(function () {
                        navigateToCreateForm1099Misc(business, EDIT, submissionId, recordId, recipientId);
                    });

                    var btnViewStatusId = "btnViewStatusId" + index;
                    $("#" + btnViewStatusId).click(function () {
                        form1099MiscViewStatus(submissionId);
                    });
                    ...
                    $("#" + btnDeleteId).click(function () {
                        deleteForm1099Misc(submissionId, recordId);
                    });
...
//  Form1099Misc status Transmit
function form1099MiscViewStatus(submissionId) {
    $("#pageProgressBar").show();

    var request = { submissionId: submissionId };
    $.ajax({
        url: form1099MiscStatusEndPoint,
        type: 'POST',
        data: JSON.stringify(request),
        contentType: 'application/json; charset=utf-8',
        success: function (response) {
            ...
            if(is200(code)) {
                ...
                $("#listForm1099MiscStatusTBody").html(htmlStatus);
...
//  Request draft Pdf URLs of the Form1099Misc Transmit
...
//  Request Pdf URLs of the Form1099Misc Transmit
function requestPdfURLs(submissionId, recordId) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var data = response.data;
            var form1099MISCRecords = data.form1099MISCRecords;
            var errorRecords = form1099MISCRecords.errorRecords;
            var successRecords = form1099MISCRecords.successRecords;
...
//  Delete Form1099Misc
function deleteForm1099Misc(submissionId, recordId) {
    $("#pageProgressBar").show();
...
function updateView(type) {
    if(isInitial(type)) {
        $("#emptyForm1099MISCDiv").hide();
        $("#form1099MiscListDiv").hide();
        $("#form1099MiscListTable").hide();
        $("#backForm1099MiscList").hide();
    } else if(isAvailableData(type)) {
        $("#emptyForm1099MISCDiv").hide();
        $("#form1099MiscListDiv").show();
        $("#form1099MiscListTable").show();
        $("#backForm1099MiscList").show();
    } else {
        $("#emptyForm1099MISCDiv").show();
        $("#form1099MiscListDiv").hide();
        $("#form1099MiscListTable").hide();
        $("#backForm1099MiscList").hide();
    }
